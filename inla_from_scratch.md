...
\begin{align}
\log p(y|x,\theta) & \propto \sum_t \left[ y_t \log(p_t) + (1 - y_t) \log(1 - p_t) \right] \\ 
...
\log p(y,x,\theta) = & \log p(y|x,\theta) + \log p(x|\theta) + \log p(\theta) \\
 \propto &\sum_t \left[ \beta x_t y_t - \log(1 + \exp(\beta x_t)) \right] + \frac12 \log|Q| - \frac12 x'Qx + \log p(\theta)
\end{align}
...
- **observations**: $p(y|x,\theta) = \prod_i p(y_i| x_i, \theta)$, observations are generally assumed to be from the exponential family
- **latent variables**: $\log p(x|\theta) \propto \frac12 \log|Q(\theta)| - \frac12 x'Qx$ where $Q$ is a sparse matrix so $x$ is a Gauss Markov random field
- **hyperparameters**: $\theta \sim p(\theta)$ is a small number of hyperparameters ($< 6$)



## Approximating the posterior $p(\theta | y)$

From the factorisation of the joint distribution
...
\begin{equation}
\log p(\theta | y) \propto \log p(y,x,\theta) - \log p(x|y,\theta)
\end{equation}
...
\begin{equation}
\log p(\theta | y) \propto \left[ \log p(y,x,\theta) - \log p(x|y,\theta) \right]_{x=x_0(\theta)}
\end{equation}

This relationship can be used to obtain the posterior $p(\theta | y)$ up to a multiplicative constant, so the result will have to be normalised so that the posterior density integrates to 1 over all $\theta$.
...
The challenge is now to find a good approximation of the conditional distribution of the latent variables $p(x|y,\theta)$.
In the INLA methodology, $p(x|y,\theta)$ is approximated using the Laplace approximation.
...
It should be emphasised that $f(x)$ is a different function for different choices of $\theta$ during the approximation of $p(\theta|y)$.
The conditional $p(x|y,\theta)$ can always be expressed through $f(x)$ by

\begin{equation}
p(x|y,\theta) = \frac{\exp[f(x)]}{\int dx \exp[f(x)]}
\end{equation}

Integrals of the form $\int dx e^{f(x)}$ can be approximated by the Laplace approximation:

\begin{equation}
\int dx \exp[f(x)] \approx \exp[f(x_0)] (2\pi)^{n/2} \big|-Hf(x_0)\big|^{-1/2}
\end{equation}
...
We now use the R package `INLA` to infer the posterior $p(\alpha|y)$.
