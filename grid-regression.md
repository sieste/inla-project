...
The precision matrix will be specified next.
...
We have estimated the posterior mode of $x$ given the observed values of $y$ and setting $\sigma^2 = 0.1$.
For later comparison, we also fit a linear regression model to each grid point individually without considering any smoothness of the regression parameters over neighboring grid points.
...
# calculate mean squared errors at each grid point
t2m_lm_mse =
...
**TODO:** 
- explore $\sigma^2$ to estimate $p(\theta \vert y)$
